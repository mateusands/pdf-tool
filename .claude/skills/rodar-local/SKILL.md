...
- **Word → PDF falha com mensagem pedindo LibreOffice** → é o `docx_convert.py` funcionando como deveria.
...
- **LibreOffice converte "em silêncio" e nada aparece** → normalmente é conflito de perfil, quando já há
  uma janela do LibreOffice aberta. O `docx_convert.py` já contorna isso com `-env:UserInstallation`
  apontando para um perfil temporário. Se voltar a acontecer, é aí que se olha.

- **A UI parece diferente do Windows** → a fonte `Segoe UI` (em `theme.py`) não existe no Linux e o Tk faz
...
- **Status bar** acompanha (`⏳` durante, `✓`/`✗` no fim) — nada de status parado em "Processando"
...
```bash
pyinstaller --onefile --windowed --add-data "theme.py:." pdf_tool.py
```
